...
def save_tsdf(args, scan, save_mesh=True):
    ...
    if args.n_view>0:
        ...
        print(n_view)
        views = [i for i in range(n_view)]
    vol_bnds = np.zeros((3, 2))

    for id in views:
        if args.test_view is not None:
            data_tmp = np.load(os.path.join(args.root_dir, "depth", scan, "refview{}.npy".format(id)), allow_pickle=True).item()
        else:
            if os.path.exists(os.path.join(args.root_dir, "depth", scan, "refview{}.npy".format(id))) == False:
                continue
            data_tmp = np.load(os.path.join(args.root_dir, "depth", scan, "refview{}.npy".format(id)), allow_pickle=True).item()
    ...
    for id in views:
        ...
        color_image = read_img(os.path.join(args.root_dir, "rgb", scan, "refview{}.jpg".format(id)))
