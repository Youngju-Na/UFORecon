...
if __name__ == '__main__':
    ...
    mask_dir = os.path.join(args.dense_folder, 'masks')
    ...
    renamed_mask_dir = os.path.join(args.dense_folder, 'masks')
    ...
    for i in range(num_images):
        if args.convert_format:
            ...
            mask_name = images[i+1].name + '.png'
            print(mask_name)
            
            if os.path.exists(os.path.join(mask_dir, mask_name)):
                print('mask file exists')
                shutil.copyfile(os.path.join(mask_dir, mask_name), os.path.join(renamed_mask_dir, '%08d_mask.jpg' % i))
