...
class CostRegNetWeight(nn.Module):
    def __init__(self, in_channels, base_channels):
        ...
        self.features = nn.Conv3d(base_channels, 8, kernel_size=3, stride=1, padding=1, bias=False)

        # Additional layer for adaptive weight volume with a single channel
        self.weights = nn.Conv3d(base_channels, 1, kernel_size=3, stride=1, padding=1, bias=False)
