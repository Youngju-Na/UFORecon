...
def get_coord_ref_ndc(src_poses, pts_3D, near_far=None, lindisp=False, extract_similarity=False, pad=False):
    '''
        Warp the provided position to the reference coordinate, and normalize to NDC coordinate.
        pts_3D [batch, N_rays N_sample 3]
    '''
    B, NV, _, _ = src_poses.shape
    
    if extract_similarity:
        pts_3D = rearrange(pts_3D, "xyz DimX -> 1 xyz DimX")
        pts_3D = repeat(pts_3D, "B xyz DimX -> B NV xyz DimX", NV=NV).float()
        pts_3D = torch.cat([pts_3D, torch.ones_like(pts_3D[:,:,:,:1])], axis=3) #* homogeneous
        
        points_in_pixel = torch.bmm(rearrange(src_poses, "B NV M_1 M_2 -> (B NV) M_1 M_2", M_1=4, M_2=4), 
                            rearrange(pts_3D, "B NV xyz DimX -> (B NV) DimX xyz")) #* matrix size가 안맞아서 생기는 문제
        
        points_in_pixel = rearrange(points_in_pixel, "(B NV) DimX xyz -> B NV DimX xyz", NV=NV)
        points_in_pixel = points_in_pixel[:,:,:3] #* homogeneous
        
        # in 2D pixel coordinate
        mask_valid_depth = points_in_pixel[:,:,2]>0  #B NV RN SN
        mask_valid_depth = mask_valid_depth.float()
        points_in_pixel = points_in_pixel[:,:,:2] / points_in_pixel[:,:,2:3] #* shape: B NV 2 xyz
        
        #* B, NV, RN, SN, 2
        point_samples_pixel = points_in_pixel.permute(0,1,3,2) #* B, NV, xyz, 2  
        return point_samples_pixel
    
    pts_3D = repeat(pts_3D, "B RN SN DimX -> B NV RN SN DimX", NV=NV).float()
    pts_3D = torch.cat([pts_3D, torch.ones_like(pts_3D[:,:,:,:,:1])], axis=4) #* homogeneous
    
    B, NV, RN, SN, _ = pts_3D.shape
    # wrap to ref view
    # B NV 4 4 -> (B NV) 4 4
    points_samples_pixel = torch.bmm(rearrange(src_poses, "B NV M_1 M_2 -> (B NV) M_1 M_2", M_1=4, M_2=4), 
                            rearrange(pts_3D, "B NV RN SN DimX -> (B NV) DimX (RN SN)")) #* 4x4x(RN*SN)
    
    points_samples_pixel = rearrange(points_samples_pixel, "(B NV) DimX (RN SN) -> B NV DimX RN SN", B=B, RN=RN)
    points_samples_pixel = points_samples_pixel[:,:,:3] #* IS this NDC coordinate?
    
    # in 2D pixel coordinate
    mask_valid_depth = points_samples_pixel[:,:,2]>0  #B NV RN SN
    mask_valid_depth = mask_valid_depth.float()
    points_samples_pixel[:,:,:2] = points_samples_pixel[:,:,:2] / points_samples_pixel[:,:,2:3] #* shape: B NV 2 RN SN
    #! near_far normalization
    if near_far is not None:
        near, far = near_far
        if lindisp:
            points_samples_pixel[:,:,2] = (1.0/points_samples_pixel[:,:,2]-1.0/near)/(1.0/far - 1.0/near)
        else:
            points_samples_pixel[:,:,2] = (points_samples_pixel[:,:,2] - near) / (far - near)  # normalize to 0~1
            points_samples_pixel[:,:, 2] = points_samples_pixel[:,:, 2] * 2 - 1.0  # normalize to -1~1
    
    
    #* B, NV, RN, SN, 2
    points_samples_pixel = points_samples_pixel.permute(0,1,3,4,2) #* B, NV, RN, SN, 2  
    points_in_pixel = points_samples_pixel[:,:,:,:,:2] #* B, NV, RN, SN, 2
    return points_in_pixel, points_samples_pixel, mask_valid_depth
