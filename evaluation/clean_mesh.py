...
def read_cam_file(filename, scale_factor=None):
    """
    ...
    #TODO: test: extrinsic *= self.scale_factor
    if scale_factor is not None:
        extrinsics *= 1.0/scale_factor
...
def clean_points_by_mask(args, points, scan, imgs_idx=None, minimal_vis=0, mask_dilated_size=11, scale_factor=None):
    cameras=[]
    mask_lis=[]
    ...
    for vid in imgs_idx:
        ...
        P = read_cam_file(proj_mat_filename, scale_factor=scale_factor)
        cameras.append(P)
...
def clean_mesh_faces_by_mask(args, mesh_file, new_mesh_file, scan, imgs_idx, minimal_vis=0, mask_dilated_size=11, scale_factor=None):
    old_mesh = trimesh.load(mesh_file) # load old mesh
    old_vertices = old_mesh.vertices[:]
    old_faces = old_mesh.faces[:]
    
    print("vertices:", old_vertices)
    print("faces", old_faces)
    
    mask = clean_points_by_mask(args, old_vertices, scan, imgs_idx, minimal_vis, mask_dilated_size, scale_factor=scale_factor)
...
def clean_mesh_faces_outside_frustum(args, scan, old_mesh_file, new_mesh_file, imgs_idx, H=1200, W=1600, mask_dilated_size=11,
                                     isolated_face_num=500, keep_largest=True, scale_factor=None):
...
if __name__ == "__main__":
    ...
    for scan in scans:
        ...
        old_mesh_file = glob(os.path.join(args.out_dir, "scan%d.ply"%scan))

        # if old_mesh_file is empty, skip
        if len(old_mesh_file) == 0:
            print("scan%d is empty" % scan)
            continue
        
        old_mesh_file = old_mesh_file[0]
        ...
        clean_mesh_faces_by_mask(args, old_mesh_file, clean_mesh_file, scan, imgs_idx, minimal_vis=1,
                                 mask_dilated_size=mask_kernel_size, scale_factor=args.scale_factor)
        clean_mesh_faces_outside_frustum(args, scan, clean_mesh_file, final_mesh_file, imgs_idx, mask_dilated_size=mask_kernel_size, scale_factor=args.scale_factor)
